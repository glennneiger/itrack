 ...
 * //===connecting with postgres and some useful command
	/*
	sudo -i -u postgres
	psql -d postgres
	\q
 ...
 */
class class_push_location{
       function set_location($data_location_array)
       {
            //============================//
            ...
            if($flag_db_conn==1)
            {
                ...
                //------------------------PUSH CHAURAHA---------------------------
                //before push road ,first get max id and max feature_id from table road
                ...
                $param_location=array();	
                $solr_location_param=array();
                ...
                $placetype='GisFeature';
                $country_name="India";
                $modificationdate=$date;	
                */
                foreach($data_location_array as $data_junction)
                {
                    ...
                    $feature_code=$data_junction['feature_code']; //ST
                    $placetype=$data_junction['placetype']; //GisFeature
                    $countryCode=$data_junction['countrycode'];
                    $source=$data_junction['source'];
                    $country_name=$data_junction['countryname'];
                    
                    $modificationdate=$date;
                    $param_location[]=array('countryCode'=>$countryCode,'source'=>$source,'location'=>$location_base,'name'=>$location_name,'featurecode'=>$feature_code,'featureclass'=>$feature_class,'modificationdate'=>$modificationdate,'lat'=>$lat_new,'lng'=>$lng_new,'country_name'=>$country_name,'placetype'=>$placetype);	
                }
                //===================================================================//	
                //print_r($param_location);
                $push_location_result=push_location($param_location,$new_max_fid,$new_max_id);//paramters
                return $push_location_result;
            }
       }
       //=== into Database table road===//	
	function push_location($param_location_arr,$new_max_fid,$new_max_id)
	{
		global $db_connection;
		global $solr_location_param;
                $result_return_db="Db insertion Not Successfull";
		foreach($param_location_arr as $param_row)
		{
			...
			$query_string="insert into gisfeature (countrycode, featureid, featureclass, featurecode, location, name, modificationdate, source, id) values ('$countryCode', $new_max_fid, '$featureclass', '$featurecode', '$location', '$name', '$modificationdate', '$source', $new_max_id)";
			...
			else 
			{
			  ...
			  $result_return_db="DB Records created successfully";			
				...
				$solr_location_param[]=array('country_code'=>$countryCode,'country_flag_url'=>"/images/flags/IN.png",'feature_id'=>$new_max_fid,'country_name'=>$country_name,'lat'=>$lat,'lng'=>$lng,'name'=>$name,'fully_qualified_name'=>$name,'feature_class'=>$featureclass,'feature_code'=>$featurecode,'placetype'=>$placetype,"timezone" => "","google_map_url"=>"","yahoo_map_url"=>"");
		...
		$solr = new Apache_Solr_Service( '127.0.0.1', '8080', '/solr' );
		  ...
		  foreach ( $solr_location_param as $item => $fields ) {
