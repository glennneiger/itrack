...
/*$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    ...
                    "vehicle_grouping.account_id='$account_id' AND vehicle.status=1 AND vehicle_assignment.status=1 AND vehicle_grouping.status=1 AND vehicle.vehicle_name IN('DL1LS9090')";*/			
...
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    "vehicle.vehicle_id = vehicle_assignment.vehicle_id AND vehicle_assignment.vehicle_id = vehicle_grouping.vehicle_id AND ".
                    "vehicle_grouping.account_id='$account_id' AND vehicle.status=1 AND vehicle_assignment.status=1 AND vehicle_grouping.status=1 and vehicle.vehicle_id IN(3398,3400)";					
*/
/*
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    "vehicle.vehicle_id = vehicle_assignment.vehicle_id AND vehicle_assignment.vehicle_id = vehicle_grouping.vehicle_id AND ".
...
//////////////////////////////////////////
$startdate = $previous_date." 06:00:00";               //TIME 6AM TO 10AM
$enddate = $current_date." 10:00:00"; 
/////////////////////////////////////////
...
//echo "\nCustomerInput=".sizeof($customer_input);
...
//if($vehicle_id_final!="")
{
	//$inc_serial=$i+1;
	$inc_serial = rand();
	$filename_title = 'BETA_VTS_HALT_REPORT_(MOTHER_ICECREAM)_'.$previous_date."_".$inc_serial;
	$fullPath = "/var/www/html/vts/beta/src/php/download/".$filename_title;
	//$fullPath = "C:\\xampp/htdocs/daily_report/motherdairy/ice_cream/excel_data/".$filename_title;
	...
	$worksheet2->write($r, 21, "Temperature In", $text_format);				//19
	$worksheet2->write($r, 22, "Temperature Out", $text_format);			//20
	
	$worksheet2->write($r, 23, "Temperature Min", $text_format);			//21
	$worksheet2->write($r, 24, "Temperature Min Date", $text_format);		//
	$worksheet2->write($r, 25, "Temperature Min Time", $text_format);		//
	$worksheet2->write($r, 26, "Temperature Max", $text_format);			//22
	$worksheet2->write($r, 27, "Temperature Max Date", $text_format);		//
	$worksheet2->write($r, 28, "Temperature Max Time", $text_format);		//
	...
	for($p=0;$p<sizeof($csv_string_halt_arr);$p++)
	{
		...
		if($csv_string_halt_arr[$p]!="")
		{
			...
			for($i=0;$i<sizeof($sheet2_row_tmp);$i++)
			{
				...
				$halt_in_temperature_temp[] = $sheet2_data_tmp[19];
				$halt_out_temperature_temp[] = $sheet2_data_tmp[20];				
				
				$temperature_min_temp[] = $sheet2_data_tmp[21];
				$temperature_min_date_temp[] = $sheet2_data_tmp[22];
				$temperature_min_time_temp[] = $sheet2_data_tmp[23];
				$temperature_max_temp[] = $sheet2_data_tmp[24];
				$temperature_max_date_temp[] = $sheet2_data_tmp[25];				
				$temperature_max_time_temp[] = $sheet2_data_tmp[26];
	...
	{				
		...
		$km[$n] = $km_temp[$i];												
		$halt_in_temperature[$n] = $halt_in_temperature_temp[$i];
		$halt_out_temperature[$n] = $halt_out_temperature_temp[$i];
		
		$temperature_min[$n] = $temperature_min_temp[$i];
		$temperature_min_date[$n] = $temperature_min_date_temp[$i];
		$temperature_min_time[$n] = $temperature_min_time_temp[$i];
		$temperature_max[$n] = $temperature_max_temp[$i];
		$temperature_max_date[$n] = $temperature_max_date_temp[$i];
		$temperature_max_time[$n] = $temperature_max_time_temp[$i];
	...
	{						
		...
		{	
			...
			if($km[$i]=="")	$km[$i]="-";
			if($halt_in_temperature[$i]=="") 	$halt_in_temperature[$i]="-";
			if($halt_out_temperature[$i]=="")	$halt_out_temperature[$i]="-";
			if($temperature_min[$i]=="") $temperature_min[$i]="-";
			if($temperature_min_datetime[$i]=="") $temperature_min_datetime[$i]="-";
			if($temperature_max[$i]=="") $temperature_max[$i]="-";
			if($temperature_max_datetime[$i]=="") $temperature_max_datetime[$i]="-";			
			...
			if($type_str[$i]!="Plant")
			{
				$relative_tmp_string = binary_plant_search($station_no[$i], $relative_customer_input, $relative_plant_input, $relative_transporter_input, $relative_route_input);				
			...
			*/
			...
			if($substr_count == 0)
			{											
				$csv_string_halt_final = $csv_string_halt_final.$vname[$i].','.$sno[$i].','.$station_no[$i].','.$type_str[$i].','.$relative_routes.','.$report_shift[$i].','.$arrivale_time[$i].','.$depature_time[$i].','.$schedule_in_time[$i].','.$time_delay[$i].','.$hrs_min[$i].','.$report_date1[$i].','.$report_time1[$i].','.$report_date2[$i].','.$report_time2[$i].','.$transporter_name_master[$i].','.$relative_transporters.','.$relative_plants.','.$km[$i].','.$halt_in_temperature[$i].','.$halt_out_temperature[$i].','.$temperature_min[$i].','.$temperature_min_date[$i].','.$temperature_min_time[$i].','.$temperature_max[$i].','.$temperature_max_date[$i].','.$temperature_max_time[$i];
				$substr_count =1;  
			}
			else
			{
			...
			//echo "<br>CSV_STRING=<br>".$csv_string_halt_final;
	...
	for($q=0;$q<sizeof($sheet2_row);$q++)
	{
		...
		{								
			...
			for($i=0;$i<sizeof($transporter_arr);$i++)
			{
				...
				{
					//echo $transporter_arr1[$j].','.$transporter_arr[$i]."\n";
		...
		{           
			...
			{					
				...
				else if($m==12 || $m==14 || $m==23 || $m==26)	//REPORT TIME, TEMP -MIN,MAX TIME
				{																			
					$tmp_date = "1970-01-01";
					$tmp_date = $tmp_date." ".$sheet2_data[$m];
				...
				else if($m==11 || $m==13 || $m==22 || $m==25)	//REPORT DATE, TEMP-MIN,MAX DATE 
				{																			
		...
		if($data_flag)
		{
			...
			$type_arr[$q] = $sheet2_data[3];	//customer no
    ...
    {    
      ...
      if(!$found)
      {
		...
		{
			if($customer_no[$m] == $relative_customer_input[$m2])
			{		  
